...
import {getMMSProductBySlug } from "@/data/loaders";
import ProductDetailClient from "@/components/productdetailedclient";
import { headers } from "next/headers";


export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const product: ProductDetail = await getMMSProductBySlug(params.slug);

  const headersList = headers();
  const host = headersList.get('host');
  const protocol = headersList.get('x-forwarded-proto') || 'http';
  const canonicalUrl = `${protocol}://${host}/maritimemaintenancesimulators/${params.slug}`;
